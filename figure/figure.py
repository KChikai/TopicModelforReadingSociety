...
def main():
    ...
    # corpus
    if os.path.exists('ap_dic.txt') and os.path.exists('ap_corpus.mm'):
        # load corpus
    ...
    # show a bar graph
    bar_chart(topics=lda_topics, topic_dim=100)
    two_bar_chart(topics1=lda_topics, topics2=hdp_topics, label1='lda', label2='hdp')

    # show the difference in the number of topics between Single Core and Multi Core
    lda = models.LdaMulticore(corpus=corpus, id2word=dictionary, num_topics=100, alpha=0.00001)
    d_topics = [lda[c] for c in corpus]
    lda = models.LdaModel(corpus=corpus, id2word=dictionary, num_topics=100, alpha=0.00001)
